...
function handleResponse(data) {
  ajaxCall(
    "POST",
    "https://us-central1-societystore.cloudfunctions.net/moduleExports/seamlessBasic/result",
    data
  )
    .then(function (data) {
...
$(document).ready(() => {
  ...
  displayFormElement(paymentOptions.upi);
  ...
  seamlessBasicForm.on("submit", function (event) {
    ...
    seamlessBasicForm
      .find("#payBtn")
      .prop({ disabled: true, class: "btnDisabled" });
    ...
    ajaxCall(
      ...
      generateDataBody(formObj, paymentTypeEnum.seamlessbasic)
    )
      .then(function (data) {
        ...
        switch ($('input[name="paymentOption"]:checked').val()) {
          case paymentOptions.card: {
            formObj.card = cFO(
              seamlessBasicForm.find("#cardForm").find("input").serializeArray()
            );
          ...
          case paymentOptions.nb: {
            formObj.nb = cFO(
              seamlessBasicForm.find("#nbForm").find("input").serializeArray()
            );
          ...
          case paymentOptions.upi: {
            formObj.upi = cFO(
              seamlessBasicForm.find("#upiForm").find("input").serializeArray()
            );
          ...
          default: {
            ...
            throw {
              name: "unsupported payment option",
              message: "selected payment option is not supported",
            };
          }
        }
